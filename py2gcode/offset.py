...
class Offset(Meta):
    def __init__(self, t, offset):#, x_start, y_start):
        ...
        while i < len(t.points):
            ...
            i_prev = -1
            while dist(s, s_prev) < 0.1:
                i_prev -= 1
                s_prev = get_from_ring(t.points, i, i_prev)
                s_prev = Point(s_prev['x'], s_prev['y'])            
            
            if s_prev == first_prev: #контур замкнулся
                break            
            ...
            i_past = 1
            while dist(s, s_past) < 0.1:
                i_past += 1
                s_past = get_from_ring(t.points, i, i_past)
                s_past = Point(s_past['x'], s_past['y'])
...
if __name__ == '__main__':
    ...
    preview2D([o], 5, options={'hideRef': True})

    def f():
        G0(0, 200, 5)
        c = Strategy()
        c.mill(o, x=0, y=0, scale = 0.1, options={'z': -10})
        #G0(Z=5)
